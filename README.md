...
2. **타겟 정의**
   - 프로젝트 목적 구체화하고 그에 맞게 타겟 변수의 값 이진화 작업 수행

3. **EDA**
   - 각 개별 변수 기술 통계량 확인, 상관성이 높은 변수와 관련성 파악
   - 새로운 인사이트
      - 등급별 대출 금액 및 소득 대비 대출 비율 분석
      - 대출 기간 및 대출 목적 분석
      - 지역별 및 재직 기간별 불량 대출률 분석
...
6. **모델링/하이퍼파라미터튜닝**
   ...
   - 안정된 예측력을 위해 트리기반 알고리즘 모델인 RandomForest, XGBoost, CatBoost, LightGBM 을 후보 변수로 하여 평가
...
7. **Tableau 대시보드 구축**
   ...
   - 두개의 파트로 구성
      ...
      - 고객의 신용 프로파일을 분석해 대출 가능 여부 파악하는 개인 신용 요인 분석 시스템
